...
def read_file_cnpj():
    ...
    cadastros_cnpj= pd.read_json('Dados/infos_cnpj_OFICIAL.json')
...
with open('Dados/contratos_OFICIAL.json', 'r', encoding='utf-8') as file:
    data = json.load(file)
...
contratos_merged.to_csv('Dados/contratos_ordenados_completo.csv', index=False) # ajustei estava ('contratos_ordenados_completos.csv')
